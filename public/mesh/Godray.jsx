  ...
  return 	mix(
    ...
    float random2D(vec2 value)
    {
      ...
      void main() {
          ...
          modelPosition.y += sin(smoothstep(0.3, vUv.y - 6., position.y) * 1.) * sin(time * 48.);
  ...
  return 	mix(
    ...
    float random2D(vec2 value)
    {
      ...
      void main()
      {
        ...
        float falloff = smoothstep(0., 1.5, vUv.y);
         float pattern = clamp((abs(vUv.x - .5)- .2) * 3.f, 0.f, 1.f) * 1.f;
        vec3 lightColor = vec3(0.7, 0.05, 0.98);
        float noise = clamp(pnoise(vNormal * 2. - time * 10. ), 0.f, 2.f);
        vec3 color = smoothstep(1., noise, vUv.y) * lightColor;
        gl_FragColor = vec4(color, color * falloff * 10.);
